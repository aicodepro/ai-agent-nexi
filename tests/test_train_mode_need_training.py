...
def test_train_mode_need_profile_flow(monkeypatch, tmp_path):
    ...
    assert "Cognitive training active" in train_mode.handle_training_command("train Nexi for coding")
...
def test_train_mode_ultra_commands(monkeypatch, tmp_path):
    ...
    assert "Ultra training active" in train_mode.handle_training_command("train Nexi deeply for sales")
