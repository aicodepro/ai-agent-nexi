...
def test_hotkey_env_disabled_by_default(monkeypatch):
    """NEXI_HOTKEY_WAKE_ENABLED must default to false."""
    monkeypatch.delenv("NEXI_HOTKEY_WAKE_ENABLED", raising=False)
    monkeypatch.delenv("NEXI_HOTKEY_ENABLED", raising=False)
    val = os.getenv("NEXI_HOTKEY_WAKE_ENABLED", "false").strip().lower()
    assert val in {"0", "false", "no", "off"}, (
        f"Expected NEXI_HOTKEY_WAKE_ENABLED to default to false, got {val}"
    )


def test_hotkey_wake_not_started_without_explicit_enable(monkeypatch):
    """When NEXI_HOTKEY_WAKE_ENABLED is false, hotkey listener is None."""
    monkeypatch.delenv("NEXI_HOTKEY_WAKE_ENABLED", raising=False)
    monkeypatch.delenv("NEXI_HOTKEY_ENABLED", raising=False)
