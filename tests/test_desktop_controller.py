...
from engine.control.desktop_controller import (
    handle_open_app, handle_close_app, handle_focus_app, handle_list_apps
)
from engine.control.process_controller import KNOWN_APPS
...
class TestDesktopController(unittest.TestCase):
    ...
    @patch("engine.control.process_controller.subprocess.Popen")
    @patch("engine.control.process_controller.os.system", side_effect=AssertionError("os.system used"))
    def test_handle_open_app_known(self, _system, mock_popen):
    ...
    @patch("engine.control.process_controller.subprocess.Popen")
    @patch("engine.control.process_controller.os.system", side_effect=AssertionError("os.system used"))
    def test_handle_open_app_chrome(self, _system, mock_popen):
    ...
    @patch("engine.control.process_controller.subprocess.run")
    ...
    @patch("engine.control.process_controller.subprocess.run")
    def test_close_app_reports_taskkill_failure(self, mock_run):
        mock_run.return_value = MagicMock(returncode=1, stderr="not found")
        result = handle_close_app(app_name="notepad")
        self.assertFalse(result.ok)
        self.assertEqual(result.error["code"], "KILL_FAILED")
    ...
    @patch("engine.control.desktop_controller.list_running")
    def test_handle_list_apps(self, mock_list):
        mock_list.return_value.ok = True
        result = handle_list_apps()
        self.assertTrue(result.ok)

    @patch("engine.control.desktop_controller.win32gui")
