...
| 5 | **Incomplete Refactor: engine/** | 🟡 MEDIUM | Incomplete refactored structure. Dead code. |
...
| 2 | **Dead code** | `engine/`, old clap backends, legacy hotword path |
...
| **Dead code paths** | 2 | `engine/`, legacy hotword fallback |
...
**However, it suffers from severe technical debt.** The monolithic `command.py` (2146 lines), 4 redundant clap backends, 5 overlapping memory systems, incomplete `engine/` refactor, and lack of tests make it difficult to maintain and extend. Security issues (cookies.json, Base64 credentials) are serious concerns.
