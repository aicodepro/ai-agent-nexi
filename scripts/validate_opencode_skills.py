"""Validate all OpenCode created skills: folder existence, SKILL.md, frontmatter, required sections, forbidden strings, index, and router."""
import os
import re
import sys
...
FORBIDDEN_PATTERNS = [
    ...
    ("curl | pipe to shell", r"curl\s+\S+\s*\|", False),
    ("shell=True instruction", r"shell\s*=\s*True", False),
...
def validate_skill(skill_name, required_sections, forbidden_patterns, check_forbidden=True):
...
def main():
    ...
    FAIL = 0
    print("=" * 60)
    print("OPENCODE ALL SKILLS VALIDATION")
    print("=" * 60)
    ...
    check("OPENCODE_SKILL_ROUTER_GUIDE.md exists",
