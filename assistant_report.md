...
| 12 | Reflection Memory | Learn from outcomes | `reflection_engine` + `reflection_memory` (observed storing live) | PARTIAL | Stores reflections; not yet voice-queryable ("what did you learn?"). |
| 13 | Procedure / Skill Library | Reusable workflows | `workflow_manager`, `local_skills` | PARTIAL / **DEFERRED (replay)** | Ephemeral workflows; durable record/replay deferred (state surface, signature drift). |
