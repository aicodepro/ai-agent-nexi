...
| Brain | Gemini Flash default; Groq intent planner/router; optional provider registry under `src/orin/brain` |
| Safety | `engine/safety_gate.py`, `src/orin/control/safety.py`, EmergencyStop, SandboxPolicy |
...
| Module | Role |
|---|---|
| `app/phase3_command_bridge.py` | Bridge from legacy command flow into ORIN systems. |
...
| `brain/self_reflection.py` | ORIN self-reflection helpers. |
...
| `memory/conversation_buffer.py` | ORIN conversation buffer. |
...
| `JARVIS_AUTO_FOLLOWUP_AFTER_TTS` | `false` |
...
| 4 | Diagnostics | `diagnostics.py`, ORIN doctors | `tests/test_diagnostics.py`, `tests/test_engine_diagnostics.py` |
